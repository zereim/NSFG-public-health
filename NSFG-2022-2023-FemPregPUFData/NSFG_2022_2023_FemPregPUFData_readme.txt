...
SAS
In SAS, add the option ‘guessingrows’ to the import statement. The 2022-2023 NSFG Female Pregnancy File has 8247
observations and the example below specifies to use 4,000 observations to determine the length:

proc import datafile="NSFG_2022_2023_FemPregPUFData.csv" out=female dbms=csv; getnames=yes; guessingrows=MAX; run;
...
the entire 2022-2023 Female Pregnancy CSV file and does not convert text strings into factors but leaves them as 
text strings. 

2022_2023_FemPregData <- read.csv("NSFG_2022_2023_FemPregPUFData.csv", stringsAsFactors=FALSE)

------

R Library(readr)
The read_csv function by default only reads in 1,000 records to determine variable attributes and will not 
convert strings to factors. If using read_csv, add the argument ‘guess_max.’ The 2022-2023 NSFG Female Respondent file has 
8247 observations and the example below specifies to use 8247 observations to determine the data type.

2022_2023_FemPregData <- read_csv("NSFG_2022_2023_FemPregPUFData.csv", guess_max = Inf)
