...
SAS
In SAS, add the option ‘guessingrows’ to the import statement. The 2022-2023 NSFG Female Respondent File has 5586 
observations and the example below specifies to use 2,000 observations to determine the length:

proc import datafile="NSFG_2022_2023_FemRespPUFData.csv" out=female dbms=csv; getnames=yes; guessingrows=MAX; run;
...
the entire 2022-2023 Female Respondent CSV file and does not convert text strings into factors but leaves them as 
text strings. 

2022_2023_FemRespData <- read.csv("NSFG_2022_2023_FemRespPUFData.csv", stringsAsFactors=FALSE)
...
5586 observations and the example below specifies to use 5586 observations to determine the data type.

2022_2023_FemRespData <- read_csv("NSFG_2022_2023_FemRespPUFData.csv", guess_max = Inf)
